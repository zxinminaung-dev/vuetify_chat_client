<template>
  <v-container fluid>
      <v-list density="compact" v-for="item in socket.messageList" :key="item" v-chat-scroll class="messages" >
        <v-list-item class="text-left mt-n4" v-if="item.to != socket.user.id" >
          <v-row>
            <v-col cols="12" md="6" sm="8">
              ...
              <v-card v-else class="mx-auto" max-width="300" height="200" :image="item.text" theme="dark"></v-card>
            </v-col>
          </v-row>
        </v-list-item>
